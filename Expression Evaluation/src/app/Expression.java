...
public class Expression {
     ...
     * to this method - you just need t o fill them in.
     */
    public static void
			...
			if (Character.isLetter(expr.charAt(i))){
				...
				while (i < expr.length()){
					if (Character.isLetter(expr.charAt(i))){
						...
						//You know that the name must be the name of an array
    ...
    evaluate(String expr, ArrayList<Variable> vars, ArrayList<Array> arrays) {
		...
		for (int i = 0; i < arr.length; i++) {
			...
			if (Character.isDigit(arr[i])) {
				...
				//We have to do seperate things for each situation
				...
				while (i < arr.length){
					if (Character.isLetter(arr[i])) {
						...
						i--; // If we dont do this the for loop will double increment i
						break;
					}
					i++;
				}
				...
				//Same thing with () we keep popping operators and applying them to numbres until we find the closest square bracket
				while (operators.peek() != '['){
					Float value1 = numbers.pop();
					Float value2 = numbers.pop();
					Float result = calculate(operators.pop(), value1, value2);
